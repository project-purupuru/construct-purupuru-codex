---
id: puruhani-earth
name: Earth Puruhani
slug: earth
entity_type: puruhani
canon_tier: canonical
element: earth
generation: kizuna
emotion_trait: Exhausted
pot_color: yellow
bear_type: brown bear
character_partner: char-nemu
source_file: entities/puruhani/earth.md
source_commit: HEAD
tags: [kizuna, puruhani, earth]
cross_references:
  - char-nemu
  - elem-earth
---

# Earth Puruhani (Exhausted)

*Name: Unknown (GAP-002)*

Sentient honey creature created by the Ooga Booga Beras (OBB) from honey. Lives in a yellow clay pot with a Brown Bear bera sticker. Paired with Nemu (Empty).

## Wuxing Mapping

| Property | Value |
|----------|-------|
| Element | Earth -- tu |
| Virtue | Fidelity -- xin |
| HENLO Letter | E |
| Color | #FCC341 (Golden Yellow) |

## Design

Soft, golden, honey-colored blob. Same species as all Puruhani but with an Exhausted expression and pose. Characteristically sleepy -- drapes over the sides of its yellow clay pot. The pot has a lid/cap. "Just woke up from centuries of hibernation" energy.

## Pair Dynamic (with Nemu)

Strangely functional. They keep each other moving as a source of reassurance. Exhausted keeps Empty going with that sliver of enthusiasm that clings after a day of endured work; Empty provides the silent company she needs. Their work despite their constantly drained mindset embodies Fidelity, the steadfast dedication to living a productive, good life.

## Origin

The OBB tried to create a new generation of bears from honey but didn't quite get things right -- the creatures had no proper jars to contain them. They were put into makeshift clay pots and buried away until the OBB could find a way to help. The Puruhani fell through from the OBB world into Tsuheji, ending up in a field of pots. Found by locals, they were quickly adopted after demonstrating their ability to inspire people. They took on quiet leadership roles, guiding early Tsuheji society toward virtue and harmony.

After believing their work was done, the Puruhani entered a deep hibernation. They were physically buried as Horai built upward over centuries. In the modern era, they were dug up at The Unearthing on the eastern outskirts of Horai.
